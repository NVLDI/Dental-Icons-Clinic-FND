...
class AuthStorageManagerClass {
  ...
  async saveUserSession(userData) {
    try {
      ...
      await Promise.all([
        ...
        SecureStore.setItemAsync('clinic_name', String(userData.clinic_name)),
        SecureStore.setItemAsync('admin_name', String(userData.admin_name)),
        SecureStore.setItemAsync('geo_location', JSON.stringify(userData.geo_location)),
  ...
  async getUserSession() {
    try {
      ...
      const geo_location = await SecureStore.getItemAsync('geo_location');
      const clinic_name = await SecureStore.getItemAsync('clinic_name');
      ...
      return {
        ...
        email,
        geo_location: geo_location ? JSON.parse(geo_location) : null,
        clinic_name, 
        isLoggedIn
      };
    } catch (error) {
  ...
  async logout() {
    try {
      ...
      await Promise.all([
        ...
        SecureStore.deleteItemAsync('geo_location'),
        SecureStore.deleteItemAsync('clinic_name'),
